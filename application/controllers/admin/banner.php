...
class Banner extends CI_controller {
    ...
    function index() {
        ...
        $data = array();
        $data['active'] = 16;
        $data['list'] = $this->db->get('banner')->result_array();
    ...
    function save() {
        ...
        $this->form_validation->set_rules('temp_file', 'Images', 'required|trim|xss_clean');
        ...
        if ($this->form_validation->run() == false) {
            ...
            if (isset($_POST['id']) && !empty($_POST['id'])) {
                $id = $_POST['id'];
                $this->db->where('id', $id);
                $this->db->update('banner', $data1);
            } else {
                $this->db->insert('banner', $data1);
            }
    ...
    function edit() {
        ...
        $data['edit'] = (array)$this->db->where('id', $id)->get('banner')->row();
    ...
    function delete() {
        ...
        $this->session->set_flashdata('msg', 'Banner has been deleted');
        redirect('admin/banner');
    }
