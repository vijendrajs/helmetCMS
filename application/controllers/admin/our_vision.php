...
class our_vision extends CI_controller {
    ...
    function save() {
       ...
       // $this->form_validation->set_rules('title', 'subject', 'required|trim|xss_clean');
        ...
        if ($this->form_validation->run() == false) {
            $this->load->view("admin/our_vision_form", $data);
        } else {
            ...
            if (isset($_POST['id']) && !empty($_POST['id'])) {
                $id = $_POST['id'];
                $this->db->where('id', $id);
                $this->db->update('our_vision', $data1);
            } else {
                $this->db->insert('our_vision', $data1);
            }
            $this->session->set_flashdata('msg', 'Data Saved');
            redirect("admin/our_vision/edit");
        }
    }
    ...
    function edit() {
        ...
        $data = array();
        $data['active'] = 2;
        $data['edit'] = (array) $this->db->where('id', 1)->get('our_vision')->row();
        $this->load->view('admin/our_vision_form', $data);
    }
    ...
    function delete() {
        ...
        $this->db->delete('our_vision');

        $this->session->set_flashdata('msg', 'data has been deleted');
        redirect('admin/our_vision');
    }

    function upload() {
        $path = 'uploads/our_vision';
        $files = $_FILES['images'];
        $title = "";
