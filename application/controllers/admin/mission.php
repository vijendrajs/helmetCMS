...
class Mission extends CI_controller {
    ...
    function save() {
       ...
       // $this->form_validation->set_rules('title', 'subject', 'required|trim|xss_clean');
        ...
        if ($this->form_validation->run() == false) {
            $this->load->view("admin/mission_form", $data);
        } else {
            ...
            if (isset($_POST['id']) && !empty($_POST['id'])) {
                $id = $_POST['id'];
                $this->db->where('id', $id);
                $this->db->update('mission', $data1);
            } else {
                $this->db->insert('mission', $data1);
            }
            $this->session->set_flashdata('msg', 'Data Saved');
            redirect("admin/mission/edit");
        }
    }
    ...
    function edit() {
        ...
        $data = array();
        $data['active'] = 2;
        $data['edit'] = (array) $this->db->where('id', 1)->get('mission')->row();
