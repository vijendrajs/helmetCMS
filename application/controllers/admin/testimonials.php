...
class testimonials extends CI_controller {
    ...
    function index() {
        ...
        $data = array();
        $data['active'] = 2;
        $data['list'] = $this->db->get('testimonials')->result_array();
    ...
    function save() {
        ...
        if ($this->form_validation->run() == false) {
            $this->load->view("admin/testimonials_form", $data);
        } else {
            ...
            if (isset($_POST['id']) && !empty($_POST['id'])) {
                $id = $_POST['id'];
                $this->db->where('id', $id);
                $this->db->update('testimonials', $data1);
            } else {
                $this->db->insert('testimonials', $data1);
            }
    ...
    function edit() {
        ...
        $data = array();
        $data['active'] = 2;
        $data['edit'] = (array) $this->db->where('id', $id)->get('testimonials')->row();
        $this->load->view('admin/testimonials_form', $data);
    }
