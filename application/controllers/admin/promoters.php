...
class promoters extends CI_controller {
    ...
    function index() {
        ...
        $data = array();
        $data['active'] = 2;
        $data['list'] = $this->db->get('promoters')->result_array();
        $this->load->view('admin/promoters_list', $data);
    }
 ...
 $this->form_validation->set_rules('status', 'Status', 'trim|xss_clean');
        ...
        if ($this->form_validation->run() == false) {
            $this->load->view("admin/promoters_form", $data);
        } else {
            ...
            if (isset($_POST['id']) && !empty($_POST['id'])) {
                $id = $_POST['id'];
                $this->db->where('id', $id);
                $this->db->update('promoters', $data1);
            } else {
                $this->db->insert('promoters', $data1);
            }
    ...
    function edit() {
        ...
        $data = array();
        $data['active'] = 2;
        $data['edit'] = (array) $this->db->where('id', $id)->get('promoters')->row();
        $this->load->view('admin/promoters_form', $data);
    }
