...
class managing_director extends CI_controller {
    ...
    function index() {
        ...
        $data = array();
        $data['active'] = 2;
        $data['list'] = $this->db->get('managing_director')->result_array();
        $this->load->view('admin/managing_director_list', $data);
    }
    ...
    function save() {
        ...
        if ($this->form_validation->run() == false) {
            $this->load->view("admin/managing_director_form", $data);
        } else {
            ...
            if (isset($_POST['id']) && !empty($_POST['id'])) {
                $id = $_POST['id'];
                $this->db->where('id', $id);
                $this->db->update('managing_director', $data1);
            } else {
                $this->db->insert('managing_director', $data1);
            }
            $this->session->set_flashdata('msg', 'Data Saved');
            redirect("admin/managing_director");
        }
    }
    ...
    function edit() {
        ...
        $data = array();
        $data['active'] = 2;
        $data['edit'] = (array) $this->db->where('id', $id)->get('managing_director')->row();
        $this->load->view('admin/managing_director_form', $data);
    }
    ...
    function delete() {
        ...
        $this->db->delete('managing_director');
    ...
    function upload() {
        $path = 'uploads/managing_director';
        $files = $_FILES['images'];
        $title = "";
